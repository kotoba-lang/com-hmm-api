(ns hmm-api.main
  "Kotodama WASM entrypoint for the Hmm_api clean-room actor (L4) — Clojure port.
  ...
  facts (`hmm_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "hmm_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem"  :plural "tradeitems"  :id-prefix "hmmapi_tra"
    :fields [:gtin :description :brand]        :required [:gtin :description]
    :coerce {}                       :refs {}}
   {:entity "Event"      :plural "events"      :id-prefix "hmmapi_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}                       :refs {}}
   {:entity "Document"   :plural "documents"   :id-prefix "hmmapi_doc"
    :fields [:type :senderId :receiverId :status] :required [:type :status]
    :coerce {}                       :refs {}}
   {:entity "Container"  :plural "containers"  :id-prefix "hmmapi_con"
    :fields [:bicCode :type :status]           :required [:bicCode :type]
    :coerce {}                       :refs {}}
   {:entity "Party"      :plural "parties"     :id-prefix "hmmapi_par"
    :fields [:gln :name :role]                 :required [:gln :name]
    :coerce {}                       :refs {}}
   {:entity "Shipment"   :plural "shipments"   :id-prefix "hmmapi_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"hmm_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hmm_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :hmm_api-compat/ready)
